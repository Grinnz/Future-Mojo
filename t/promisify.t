...
my @results;
my $p = $f->promisify->then(sub { @results = @_ });
...
my @failure;
$p = $f->promisify->catch(sub { @failure = @_ });
...
$p = $f->promisify->then(sub { @results = @_ });
...
subtest 'IO::Async::Loop::Mojo' => sub {
  ...
  Role::Tiny->apply_roles_to_object($f, 'Future::Role::Promisify');
  my $completed;
  $p = $f->promisify->then(sub { $completed++ });
  $t = $p->ioloop->timer(0.5 => sub { $p->ioloop->stop });
  $p->wait;
  $p->ioloop->remove($t);
  ...
  Role::Tiny->apply_roles_to_object($f, 'Future::Role::Promisify');
  my $failed;
  $p = $f->promisify->catch(sub { $failed++ });
  $t = $p->ioloop->timer(0.5 => sub { $p->ioloop->stop });
  $p->wait;
  $p->ioloop->remove($t);
